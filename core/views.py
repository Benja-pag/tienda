...
def productos(request, accion, id):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f'Producto "{str(producto)}" se logró {accion} correctamente')
    ...
    if request.method == 'GET':
        if accion == 'crear':
            ...
            eliminado, mesaje = eliminar_registro(Producto,id)
            messages.success(request, mesaje)
            if eliminado:
...
def usuarios(request, accion, id):
    ...
    perfil = usuario.perfil if usuario and hasattr(usuario, 'perfil') else None
    ...
    if request.method == 'POST':
        ...
        if form_usuario.is_valid() and form_perfil.is_valid():
            ...
            perfil.usuario_id = usuario.id
            perfil.save()
            messages.success(request, f'El usuario {usuario.first_name} {usuario.last_name} ha sido guardado correctamente.')
...
def bodega(request):
    if request.method == 'POST':
        # Crear una instancia del formulario de bodega con los datos del POST
        ...
        if cantidad == 1:
            ...
            messages.success(request, f'Se han agregado {cantidad} productos de {producto.nombre} a las bodega')
...
def eliminar_producto_en_bodega(request, bodega_id):
    ...
    if eliminado:
        messages.success(request, f'Se ha eliminado el id {bodega_id} ({nombre_producto})')
    else:
